...
module mac_unit (
    ...
    // The multiply tree and adder tree are now combinational only, feeding sum_reg.
    ...
    // Pipeline stage: register the multiply/tree output
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulate using registered sum instead of direct combinational path
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
