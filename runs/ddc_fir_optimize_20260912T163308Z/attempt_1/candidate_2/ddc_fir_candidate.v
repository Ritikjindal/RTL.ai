...
module ddc_fir
  #(parameter		    IQW = 16,  // input sample width (Q1.15)
   ...
   // Split reduction: psumA covers prod[0..7] (8 taps), psumB covers prod[8..14] (7 taps)
   
   // First half reduction: prod[0] through prod[7]
   ...
   // Second half reduction: prod[8] through prod[14]
   ...
   // Pipeline register for partial sums
   reg signed [ACCW-1:0] partial_sum_reg_A;
   reg signed [ACCW-1:0] partial_sum_reg_B;
   reg			   in_valid_1;  // Extended out_valid pipeline stage

   // Combined accumulator on next cycle
   ...
   always @(posedge clk) begin
      if (rst) begin
         ...
         // First stage: tap shift and partial sum capture
         if (in_valid) begin
         ...
         // Second stage: combine partial sums and output
