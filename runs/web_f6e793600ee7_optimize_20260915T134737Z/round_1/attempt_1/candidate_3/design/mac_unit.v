...
module mac_unit (
    ...
    // Everything from a_reg/b_reg through to acc is combinational - the long path.
    ...
    // Pipeline stage: capture multiply and adder-tree results
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulate using pipelined sum
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
