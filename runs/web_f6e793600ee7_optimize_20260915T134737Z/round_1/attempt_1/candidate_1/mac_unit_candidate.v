...
module mac_unit (
    ...
    // This combinational path is now broken by sum_reg.
    ...
    // Pipeline stage: capture sum and valid after the multiply and adder tree
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulate using the pipelined sum_reg
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
