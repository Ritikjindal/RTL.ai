...
module bm_fsm_ctrl #(
    ...
    // Parallel candidates for accumulator update, each computed independently from d0,
    // then selected with a one-hot mux instead of a serial ripple.
    ...
    // Parallel candidates computed from d0
    wire [ACCW-1:0] cand3  = d0 + ext;
    wire [ACCW-1:0] cand4  = d0 + {ext[ACCW-2:0], 1'b0};
    wire [ACCW-1:0] cand5  = d0 + held;
    wire [ACCW-1:0] cand6  = d0 + lenx;
    wire [ACCW-1:0] cand7  = d0 + cntx;
    wire [ACCW-1:0] cand8  = d0 + ext + cntx;
    wire [ACCW-1:0] cand9  = d0 ^ {ext[ACCW-9:0], 8'h7D};
    wire [ACCW-1:0] cand10 = d0 + held + lenx;
    wire [ACCW-1:0] cand11 = d0 ^ acc;
    
    // One-hot select to choose which candidate drives the final result
    wire [ACCW-1:0] d9;
    
    always @* begin
        d9 = d0;  // default to d0 (no update when no state matches)
        case (1'b1)
            state[3]:  d9 = cand3;
            state[4]:  d9 = cand4;
            state[5]:  d9 = cand5;
