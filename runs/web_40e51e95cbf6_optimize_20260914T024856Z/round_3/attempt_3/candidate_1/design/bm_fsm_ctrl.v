...
module bm_fsm_ctrl #(
    ...
    // Restructured combinational path: one-hot mux applied directly to d0
    // instead of a serial chain of dependent stages.
    ...
    always @* begin
        ...
        case (1'b1)                       // one-hot case
            state[3]:  d9 = d0 + ext;
            state[4]:  d9 = d0 + {ext[ACCW-2:0], 1'b0};
            state[5]:  d9 = d0 + held;
            state[6]:  d9 = d0 + lenx;
            state[7]:  d9 = d0 + cntx;
            state[8]:  d9 = d0 + ext + cntx;
            state[9]:  d9 = d0 ^ {ext[ACCW-9:0], 8'h7D};
            state[10]: d9 = d0 + held + lenx;
            state[11]: d9 = d0 ^ acc;
            default:   d9 = d0;
        endcase
    end
