...
module bm_fsm_ctrl #(
    ...
    // Deliberately an unbalanced chain of conditional adds rather than a balanced
    // structure: this is the combinational path to restructure.
    ...
    wire [ACCW-1:0] d5 = state[7] ? (d4 + cntx)           : d4;
    wire [ACCW-1:0] d6 = state[8] ? (d5 + ext + cntx)     : d5;
    wire [ACCW-1:0] d7 = state[9] ? (d6 ^ {ext[ACCW-9:0], 8'h7D}) : d6;
    wire [ACCW-1:0] d8 = state[10] ? (d7 + held + lenx)   : d7;
    wire [ACCW-1:0] d9 = state[11] ? (d8 ^ acc)           : d8;
