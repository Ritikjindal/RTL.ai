// designs/bm_fir6.v — 6-tap FIR with a serial accumulate.
...
module bm_fir6 #(
    ...
    // First level: balanced tree for first three products
    wire signed [ACCW-1:0] sa_l1 = p0 + p1;
    wire signed [ACCW-1:0] sa = sa_l1 + p2;

    // First level: balanced tree for last three products
    wire signed [ACCW-1:0] sb_l1 = p3 + p4;
    wire signed [ACCW-1:0] sb = sb_l1 + p5;

    // Pipeline registers for partial sums and valid signal
    reg signed [ACCW-1:0] sa_r, sb_r;
    reg                   vld2;
