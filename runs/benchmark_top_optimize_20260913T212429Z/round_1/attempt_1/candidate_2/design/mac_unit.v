...
module mac_unit (
    ...
    // Everything from a_reg/b_reg through to sum is combinational.
    ...
    // Pipeline stage: register the multiplier-tree output
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulate using the registered values from the previous cycle
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
