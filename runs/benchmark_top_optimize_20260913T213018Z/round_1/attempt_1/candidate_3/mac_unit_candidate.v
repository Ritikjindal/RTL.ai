...
module mac_unit (
    ...
    // This combinational path now feeds sum_reg instead of directly into acc.
    ...
    // Pipeline stage 1: register the sum and delay valid signal
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Pipeline stage 2: accumulate using registered values
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
