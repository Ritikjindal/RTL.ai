...
module mac_unit (
    ...
    // The multiply and adder-tree logic is combinational and feeds sum_reg.
    ...
    // Pipeline stage 1: capture the sum output and delay valid signal
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Pipeline stage 2: accumulate using registered sum value
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
