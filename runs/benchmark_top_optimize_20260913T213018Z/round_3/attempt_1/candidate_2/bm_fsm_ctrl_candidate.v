...
module bm_fsm_ctrl #(
    ...
    // Pipeline registers for second half of datapath
    reg [ACCW-1:0] d4_reg;
    ...
    // First half: d0 through d4 (combinational)
    ...
    // Second half: d5 through d9 (combinational), using registered versions
    wire [ACCW-1:0] d5 = state5_11_reg[7] ? (d4_reg + cntx_reg)           : d4_reg;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Pipeline stage: capture d4 and operands for second half
            d4_reg    <= d4;
            ...
            // Capture d9 result from pipelined computation
            d9_reg <= d9;

            // Use d9_reg (one cycle delayed) instead of combinational d9
            if (in_valid_d1)
                acc <= d9_reg;
            else if (state[12])
                acc <= {ACCW{1'b0}};
