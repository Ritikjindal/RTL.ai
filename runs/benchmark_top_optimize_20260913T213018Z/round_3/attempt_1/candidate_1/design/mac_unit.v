...
module mac_unit (
    ...
    // This combinational logic is now followed by a pipeline register stage.
    ...
    // Pipeline stage 1: Register the sum and valid signal
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Pipeline stage 2: Accumulate using the registered sum
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
