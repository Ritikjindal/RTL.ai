...
module bm_fsm_ctrl #(
    ...
    // Pipeline stage registers (between d4 and d5)
    reg [ACCW-1:0] d4_reg;
    reg [10:5]     state5_11_reg;  // state[5:11] registered
    reg [ACCW-1:0] ext_reg;
    reg [ACCW-1:0] held_reg;
    reg [ACCW-1:0] lenx_reg;
    reg [ACCW-1:0] cntx_reg;
    ...
    // First half: d0 through d4 (combinational, computed from current cycle)
    ...
    // Second half: d5 through d9 (combinational, computed from registered d4 and registered operands)
    wire [ACCW-1:0] d5 = state5_11_reg[5] ? (d4_reg + cntx_reg)           : d4_reg;
    wire [ACCW-1:0] d6 = state5_11_reg[8] ? (d5 + ext_reg + cntx_reg)     : d5;
    wire [ACCW-1:0] d7 = state5_11_reg[9] ? (d6 ^ {ext_reg[ACCW-9:0], 8'h7D}) : d6;
    wire [ACCW-1:0] d8 = state5_11_reg[10] ? (d7 + held_reg + lenx_reg)   : d7;
    wire [ACCW-1:0] d9 = state5_11_reg[11] ? (d8 ^ acc_reg)           : d8;

    // d9_reg will hold the pipelined result, used as the new acc value
    wire [ACCW-1:0] d9_reg_input = d9;
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Pipeline stage: capture d4 and operands for the second half
            d4_reg <= d4;
            ...
            // Update acc from pipelined d9 result, gated with delayed in_valid
            if (in_valid_d1)
                acc <= d9_reg_input;
            else if (state[12])
                acc <= {ACCW{1'b0}};
