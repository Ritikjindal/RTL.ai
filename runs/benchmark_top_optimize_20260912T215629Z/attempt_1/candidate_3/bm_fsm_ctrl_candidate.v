...
module bm_fsm_ctrl #(
    ...
    // Pipeline stage registers for second half of datapath chain
    ...
    // First half of the chain (d0 through d4)
    ...
    // Second half of the chain (d5 through d9), using delayed pipeline signals
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            acc_mid   <= {ACCW{1'b0}};
            state_d   <= 16'b0000_0000_0000_0001;  // S_IDLE
            held_d    <= {ACCW{1'b0}};
            lenx_d    <= {ACCW{1'b0}};
            cntx_d    <= {ACCW{1'b0}};
            ext_d     <= {ACCW{1'b0}};
            ...
            // Pipeline stage: capture d4 and all signals needed for d5..d9
            acc_mid  <= d4;
            state_d  <= state;
            held_d   <= held;
            lenx_d   <= lenx;
            ...
            // Update acc with the delayed d9 (conditional on delayed in_valid)
            if (in_valid_d)
                acc <= d9;
            else if (state_d[12])
                acc <= {ACCW{1'b0}};
