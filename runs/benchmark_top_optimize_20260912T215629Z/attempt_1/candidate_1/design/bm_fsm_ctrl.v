...
module bm_fsm_ctrl #(
    ...
    // First half of the chain (combinational, same cycle)
    ...
    // Second half of the chain (combinational, uses delayed signals from pipeline stage)
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // Pipeline stage: capture d4 and all signals needed by second half
            acc_mid   <= d4;
            state_d   <= state;
            held_d    <= held;
            lenx_d    <= lenx;
            ...
            // Update acc with delayed d9 if in_valid_d was true
            if (in_valid_d)
                acc <= d9;
            else if (state_d[12])
                acc <= {ACCW{1'b0}};
