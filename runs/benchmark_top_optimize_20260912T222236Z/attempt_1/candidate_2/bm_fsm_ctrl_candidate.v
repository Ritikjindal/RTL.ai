...
module bm_fsm_ctrl #(
    ...
    // Restructured to compute all operations in parallel from d0, then select
    // the result with a single one-hot mux based on state bits [3..11].
    ...
    // Compute all possible results in parallel directly from d0
    wire [ACCW-1:0] r3  = d0 + ext;
    wire [ACCW-1:0] r4  = d0 + {ext[ACCW-2:0], 1'b0};
    wire [ACCW-1:0] r5  = d0 + held;
    wire [ACCW-1:0] r6  = d0 + lenx;
    wire [ACCW-1:0] r7  = d0 + cntx;
    wire [ACCW-1:0] r8  = d0 + ext + cntx;
    wire [ACCW-1:0] r9  = d0 ^ {ext[ACCW-9:0], 8'h7D};
    wire [ACCW-1:0] r10 = d0 + held + lenx;
    wire [ACCW-1:0] r11 = d0 ^ acc;
    
    // Single one-hot mux selects final result based on state bits [3..11]
    wire [ACCW-1:0] d9 = state[3]  ? r3  :
                         state[4]  ? r4  :
                         state[5]  ? r5  :
                         state[6]  ? r6  :
                         state[7]  ? r7  :
