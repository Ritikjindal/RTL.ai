...
module mac_unit (
    ...
    // The multiply and adder tree is now combinational only up to sum_reg.
    ...
    // Pipeline stage: capture sum and valid_reg
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulator uses the pipelined sum_reg and valid_reg2
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
