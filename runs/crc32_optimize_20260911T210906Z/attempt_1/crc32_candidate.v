...
module crc32
  #(parameter DW = 64)                  // datapath bits/clock (mult of 8)
   ...
   // Mid-pipeline registers for pipelining the byte-tap chain
   reg [31:0]	     crc_mid;
   reg [DW-1:DW/2]  data_hi_reg;
   reg              last_reg;
   reg [$clog2(DW/8):0] bytes_reg;
   reg              valid_reg;

   // Stage A: Running CRC tapped at each byte boundary for first half
   // c_tap_a[n] = CRC after folding the low n bytes of in_data (first half only)
   reg [31:0]	     c_tap_a [0:HBW];
   
   // Stage B: Running CRC tapped at each byte boundary for second half
   // c_tap_b[n] = CRC after folding bytes starting from the mid-point
   reg [31:0]	     c_tap_b [0:HBW];
   
   reg [31:0]	     cnext;
   integer	     b, i;

   // Stage A combinational block: compute first half of byte fold
   always @* begin
      c_tap_a[0] = cur;
      for (b = 0; b < HBW; b = b + 1) begin
         c_tap_a[b+1] = c_tap_a[b];
         for (i = 0; i < 8; i = i + 1)
           c_tap_a[b+1] = (c_tap_a[b+1] >> 1) ^
                  (POLY & {32{c_tap_a[b+1][0] ^ in_data[b*8 + i]}});
      end
   end

   // Stage B combinational block: compute second half of byte fold
   always @* begin
      c_tap_b[0] = crc_mid;
      for (b = 0; b < HBW; b = b + 1) begin
         c_tap_b[b+1] = c_tap_b[b];
         for (i = 0; i < 8; i = i + 1)
           c_tap_b[b+1] = (c_tap_b[b+1] >> 1) ^
                  (POLY & {32{c_tap_b[b+1][0] ^ data_hi_reg[HBW*8 + b*8 + i]}});
      end
      // last word folds only its valid bytes; full words fold all HBW
      cnext = last_reg ? c_tap_b[bytes_reg - HBW] : c_tap_b[HBW];
   end
   ...
   always @(posedge clk) begin
      if (rst) begin
         ...
         // Pipeline stage 1: capture Stage A outputs and input context
         if (in_valid) begin
            crc_mid   <= c_tap_a[HBW];
         ...
         // Pipeline stage 2: use captured values to complete the fold
         if (valid_reg) begin
            crc    <= cnext;
            active <= ~last_reg;          // re-init on the next frame
            if (last_reg) begin
               out_valid <= 1'b1;
