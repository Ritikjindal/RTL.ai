// designs/bm_dot4.v — 4-element signed dot product, 12x12 multipliers.
//four wide multiplies feed a 2-level add tree and an
// accumulator, all in one clock.
`default_nettype wire

// This module owns the critical path of the full design (clock 'clk_c', worst slack -1.32 ns).
module bm_dot4 #(
    parameter integer DW   = 8,
    parameter integer PW   = 2 * DW,
    parameter integer ACCW = PW + 4
) (
    input  wire                   clk,
    input  wire                   rst,
    input  wire                   in_valid,
    input  wire [4*DW-1:0]        a_vec,
    input  wire [4*DW-1:0]        b_vec,
    output reg                    out_valid,
    output reg  signed [ACCW-1:0] acc
);

    reg [4*DW-1:0] a_reg, b_reg;
    reg            vld;

    always @(posedge clk) begin
        if (rst) begin
            a_reg <= {(4*DW){1'b0}};
            b_reg <= {(4*DW){1'b0}};
            vld   <= 1'b0;
        end else begin
            a_reg <= a_vec;
            b_reg <= b_vec;
            vld   <= in_valid;
        end
    end

    wire signed [DW-1:0] a0 = a_reg[1*DW-1:0*DW];
    wire signed [DW-1:0] a1 = a_reg[2*DW-1:1*DW];
    wire signed [DW-1:0] a2 = a_reg[3*DW-1:2*DW];
    wire signed [DW-1:0] a3 = a_reg[4*DW-1:3*DW];

    wire signed [DW-1:0] b0 = b_reg[1*DW-1:0*DW];
    wire signed [DW-1:0] b1 = b_reg[2*DW-1:1*DW];
    wire signed [DW-1:0] b2 = b_reg[3*DW-1:2*DW];
    wire signed [DW-1:0] b3 = b_reg[4*DW-1:3*DW];

    wire signed [PW-1:0] m0 = a0 * b0;
    wire signed [PW-1:0] m1 = a1 * b1;
    wire signed [PW-1:0] m2 = a2 * b2;
    wire signed [PW-1:0] m3 = a3 * b3;

    // Pipeline stage 1: Register the multiplier outputs
    reg signed [PW-1:0] m0_r, m1_r, m2_r, m3_r;
    reg                 vld2;

    always @(posedge clk) begin
        if (rst) begin
            m0_r <= {PW{1'b0}};
            m1_r <= {PW{1'b0}};
            m2_r <= {PW{1'b0}};
            m3_r <= {PW{1'b0}};
            vld2 <= 1'b0;
        end else begin
            m0_r <= m0;
            m1_r <= m1;
            m2_r <= m2;
            m3_r <= m3;
            vld2 <= vld;
        end
    end

    // Second-stage adder tree using registered multiplier outputs
    wire signed [PW:0]   h0  = m0_r + m1_r;
    wire signed [PW:0]   h1  = m2_r + m3_r;
    wire signed [PW+1:0] sum = h0 + h1;

    always @(posedge clk) begin
        if (rst) begin
            acc       <= {ACCW{1'b0}};
            out_valid <= 1'b0;
        end else begin
            if (vld2) acc <= acc + {{(ACCW-PW-2){sum[PW+1]}}, sum};
            out_valid <= vld2;
        end
    end

endmodule