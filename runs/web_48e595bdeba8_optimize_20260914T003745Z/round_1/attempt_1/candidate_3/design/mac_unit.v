...
module mac_unit (
    ...
    // This stage is now followed by a pipeline register before accumulation.
    ...
    // Pipeline stage: capture sum and valid flag before accumulator
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulator using the pipelined sum
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
