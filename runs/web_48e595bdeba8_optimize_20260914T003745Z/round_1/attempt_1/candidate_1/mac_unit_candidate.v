...
module mac_unit (
    ...
    // Everything from a_reg/b_reg through to sum is combinational - now split into two stages.
    ...
    // Pipeline stage 1: capture sum and valid_reg
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Pipeline stage 2: accumulate using sum_reg
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
