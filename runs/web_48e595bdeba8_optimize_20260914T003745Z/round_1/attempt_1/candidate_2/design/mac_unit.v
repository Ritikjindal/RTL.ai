...
// This module owns the critical path of the full design (clock 'clk_a', worst slack -1.41 ns).
module mac_unit (
    input  wire        clk,
    input  wire        rst,
    input  wire        valid,
    ...
    // Everything from a_reg/b_reg through to sum is combinational - first pipeline stage.
    ...
    // Pipeline register stage: capture sum and valid_reg before accumulation
    always @(posedge clk) begin
        if (rst) begin
            sum_reg    <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Second pipeline stage: accumulator consumes sum_reg
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
