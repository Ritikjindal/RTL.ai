...
module crc32
  #(parameter DW = 64)                  // datapath bits/clock (mult of 8)
   ...
   localparam HW = BW/2;                // half-width for pipelining
   ...
   // First stage: compute c_tap[0..HW] using in_data[0:HW*8-1]
   reg [31:0]	     c_tap_lo [0:HW];
   integer	     b, i;

   always @* begin
      c_tap_lo[0] = cur;
      for (b = 0; b < HW; b = b + 1) begin
         c_tap_lo[b+1] = c_tap_lo[b];
         for (i = 0; i < 8; i = i + 1)
           c_tap_lo[b+1] = (c_tap_lo[b+1] >> 1) ^
                  (POLY & {32{c_tap_lo[b+1][0] ^ in_data[b*8 + i]}});
      end
   end

   // Pipeline registers
   reg [31:0]	     crc_mid;
   reg [DW-1:HW*8]   in_data_hi_r;
   reg               in_last_r;
   reg [$clog2(DW/8):0] in_bytes_r;
   reg               in_valid_r;

   // Second stage: compute c_tap[HW..BW] using in_data_hi_r and crc_mid
   reg [31:0]	     c_tap_hi [0:HW];
   reg [31:0]	     cnext;

   always @* begin
      c_tap_hi[0] = crc_mid;
      for (b = 0; b < HW; b = b + 1) begin
         c_tap_hi[b+1] = c_tap_hi[b];
         for (i = 0; i < 8; i = i + 1)
           c_tap_hi[b+1] = (c_tap_hi[b+1] >> 1) ^
                  (POLY & {32{c_tap_hi[b+1][0] ^ in_data_hi_r[b*8 + i]}});
      end
      // last word folds only its valid bytes; full words fold all BW
      cnext = in_last_r ? c_tap_hi[in_bytes_r - HW] : c_tap_hi[HW];
   end
   ...
   always @(posedge clk) begin
      if (rst) begin
         ...
         if (in_valid) begin
            crc_mid   <= c_tap_lo[HW];
            in_data_hi_r <= in_data[DW-1:HW*8];
            in_last_r <= in_last;
            in_bytes_r <= in_bytes;
            in_valid_r <= 1'b1;
         end
