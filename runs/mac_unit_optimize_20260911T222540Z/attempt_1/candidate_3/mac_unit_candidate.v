...
module mac_unit (
    ...
    // Everything from a_reg/b_reg through to sum is combinational - the multiply+adder-tree path.
    ...
    // Pipeline stage: register the adder-tree output before accumulation
    always @(posedge clk) begin
        if (rst) begin
            sum_reg   <= 18'd0;
            valid_reg2 <= 1'b0;
    ...
    // Accumulator using the pipelined sum_reg and valid_reg2
    always @(posedge clk) begin
        if (rst) begin
            acc       <= 24'd0;
            acc_valid <= 1'b0;
