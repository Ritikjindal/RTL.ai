...
module benchmark_top (
    ...
    localparam integer N_E = 5;   // bm_butterfly  : complex MAC (FFT radix-2)
    ...
    // ============================================ domain E: bm_butterfly x N_E
    wire [23:0] e_cr [0:N_E-1];
    wire [23:0] e_ci [0:N_E-1];

    generate
        for (i = 0; i < N_E; i = i + 1) begin : g_bfly
            bm_butterfly u_bfly (
                .clk       (clk_e),
                .rst       (rst_e_rep[i]),
                .in_valid  (lfsr_e[4]),
                .ar        (lfsr_e[9:0]   ^ (10'd3  * i)),
                .ai        (lfsr_e[19:10] ^ (10'd5  * i)),
                .br        (lfsr_e[29:20] ^ (10'd7  * i)),
                .bi        ({lfsr_e[31:30], lfsr_e[7:0]} ^ (10'd11 * i)),
                .out_valid (),
                .cr        (e_cr[i]),
                .ci        (e_ci[i])
            );
    ...
    always @* begin
        e_red_r = 24'd0;
        for (k = 0; k < N_E; k = k + 1) e_red_r = e_red_r ^ e_cr[k] ^ e_ci[k];
    end
